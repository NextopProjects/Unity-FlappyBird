...
public class QLearning
{
    ...
    private int _actionCount = 2; //행동 개수 (stay, jump)

    public QLearning(float learningRate, float discountFactor, float epsilon)
    {
    ...
    public void Update((int, int) state, int action,int reward, (int, int) nextState)
    {
        // 새로운 상태고 최초라면 Q-Value 생성하고 초기화
        if (!_qTable.ContainsKey(state))
            _qTable[state] = new float[_actionCount];
